...
(async () => {
  ...
  try {
    ...
    assert.match(await changes.innerText(), /前期間: 2026-09-04/);
    assert.doesNotMatch(await changes.innerText(), /pt/);
    const changesDownloadPromise = page.waitForEvent('download');
    await changes.getByRole('button', { name: 'PNG', exact: true }).click();
    const changesDownload = await changesDownloadPromise;
    assert.equal(changesDownload.suggestedFilename(), 'period-environment-changes.png');
    const changesPath = path.join(output, changesDownload.suggestedFilename());
    ...
    assert.doesNotMatch(await tierBlock.innerText(), /自動Tier候補|最終Tier/);
    const tier2Column = tierBlock.locator('div.rounded-md').filter({ has: page.getByRole('heading', { name: 'Tier2', exact: true }) });
