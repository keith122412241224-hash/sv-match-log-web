...
- 今回の差分にDB定義、マイグレーション、ランクUI、ランク条件、集計ロジック、依存関係の変更はない。元の未コミット・未追跡ファイルは保持した。
...
ファイル名は`analysis-ブロック名-YYYYMMDD-HHmmss.png`。保存日時は利用端末のローカル時刻。
...
- 既存の`html-to-image`を動的読み込みして利用。新しいライブラリは追加しない。
- 保存中は連打を防止。失敗時は再試行できる案内を表示し、一時DOMを必ず除去。
- 大きな表は1600万画素・一辺16000pxを上限に解像度を調整。等倍でも上限を超える場合は保存を止め、期間・デッキの絞り込みを案内する。
...
コードは本番反映可能。今回の作業ではpush・本番デプロイは実行していない。
...
3. 上記6ファイルだけが今回の変更であることを確認する。ランク変更のある元の`master`をマージ・push・デプロイしない。
